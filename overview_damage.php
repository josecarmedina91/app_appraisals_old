...
$stmt = $pdo->prepare("SELECT damage FROM tb_exterior_inspecciones WHERE id_inspeccion = ?");
$stmt->execute([$inspectionId]);
if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $buttonState = array_filter(explode(', ', $row['damage']));
    if (empty($row['damage'])) {
...
$buttonOptions = ['Asbestos', 'Damage', 'Damaged Roof Covering', 'Grading Towards Foundation', 'Large Cracks in Foundation', 'Moisture/Water Leakage', 'Mould', 'UFR', 'No'];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_FILES['photo'])) {
        ...
        $folderName = "img/photo_gallery/{$inspectionId}/Damage";
    ...
    if (isset($_POST['activeButtons'])) {
        ...
        try {
            $stmt = $pdo->prepare("UPDATE tb_exterior_inspecciones SET damage = ? WHERE id_inspeccion = ?");
    ...
    if (isset($_POST['add_note'])) {
        ...
        $buttonName = $_POST['buttonName'];
        if (!empty($note)) {
            $stmt = $pdo->prepare("SELECT notas_ext_damage FROM tb_exterior_inspecciones WHERE id_inspeccion = ?");
            $stmt->execute([$inspectionId]);
            ...
            $stmt = $pdo->prepare("UPDATE tb_exterior_inspecciones SET notas_ext_damage = ? WHERE id_inspeccion = ?");
...
<body class="h-full w-full">
    <div id="initialMessage" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center <?php echo $showInitialMessage ? '' : 'hidden'; ?>">
        <div class="bg-white p-6 rounded-lg shadow-lg text-center max-w-sm mx-auto">
            <p class="text-base font-semibold mb-4">Have you observed any concerning damage or detrimental factors?</p>
            <div class="flex justify-center gap-4">
                <button onclick="handleResponse('yes')" class="flex items-center justify-center bg-green-500 text-white px-4 py-2 rounded-md hover:bg-green-600">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                    </svg>
                    Yes
                </button>
                <button onclick="handleResponse('no')" class="flex items-center justify-center bg-red-500 text-white px-4 py-2 rounded-md hover:bg-red-600">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <div id="content" class="<?php echo $showInitialMessage ? 'hidden' : ''; ?> bg-white p-4 flex flex-col items-center">
        <div class="flex items-center justify-between w-full max-w-lg border-b pb-3">
            <a href="#" onclick="saveAndGoBack()" class="text-gray-600 h-6 w-6">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    <path d="m15 18-6-6 6-6"></path>
                </svg>
            </a>
            <h1 id="super_titulo" class="hidden text-xl font-semibold text-center">Damage</h1>
            <h1 class="text-xl font-semibold text-center">Detrimental Factors</h1>
            <div style="width:24px"></div>
        </div>
        <div class="flex justify-between w-full max-w-lg mb-4 mt-4">
    ...
    <script>
        ...
        function toggleButton(button, index) {
            ...
            const anyButtonActive = Array.from(buttons).some(btn => btn.classList.contains('bg-green-300') && btn !== noButton);

            if (!anyButtonActive) {
                noButton.classList.add('bg-green-300');
                noButton.classList.remove('bg-background', 'hover:bg-accent');
            }
        ...
        function saveAndGoBack() {
            if (!changesMade) {
                window.location.href = 'overview_inspection.php?id=<?php echo $inspectionId; ?>';
                return;
            }
            const buttons = document.querySelectorAll('button[onclick^="toggleButton"]');
            let activeButtons = [];
        ...
        function addOtherOption(value) {
            ...
            document.querySelector('.flex.w-full.max-w-lg.mt-4 .flex.flex-col.w-3\\/10.pl-2.gap-4').appendChild(buttonGroup);
        ...
        function handleResponse(response) {
            if (response === 'yes') {
                ...
                const formData = new FormData();
                formData.append('activeButtons', '"No"');
                formData.append('id', '<?php echo $inspectionId; ?>');
