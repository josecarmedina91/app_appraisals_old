...
$buttonState = [];
$stmt = $pdo->prepare("SELECT windows FROM tb_interior_inspecciones WHERE id_inspeccion = ?");
$stmt->execute([$inspectionId]);
if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $buttonState = array_unique(array_filter(explode(', ', $row['windows'])));
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['activeButtons'])) {
        ...
        try {
            $stmt = $pdo->prepare("UPDATE tb_interior_inspecciones SET windows = ? WHERE id_inspeccion = ?");
    ...
    if (isset($_POST['add_note'])) {
        $note = $_POST['note'];
        if (!empty($note)) {
            $stmt = $pdo->prepare("SELECT notas_int_win FROM tb_interior_inspecciones WHERE id_inspeccion = ?");
            $stmt->execute([$inspectionId]);
            ...
            $stmt = $pdo->prepare("UPDATE tb_interior_inspecciones SET notas_int_win = ? WHERE id_inspeccion = ?");
    ...
    <div class="bg-white p-4">
        ...
        <div class="grid grid-cols-2 gap-4 mt-4">
            ...
            <div class="col-span-1">
                <h3 class="text-lg font-semibold text-center p-1">Number of Panes</h3>
                <div class="flex flex-col items-center">
                    <?php
                    $numberOfPanes = ['Single', 'Double', 'Triple'];
                    $renderedButtons = [];
            ...
            <div class="col-span-1">
                <h3 class="text-lg font-semibold text-center p-1">Frame Type</h3>
                <div class="flex flex-col items-center">
                    <?php
                    $frameTypes = ['PVC', 'Aluminum', 'Vinyl', 'Wood'];
