...
// Sustituciones
$buttonState = [];
$stmt = $pdo->prepare("SELECT closets FROM tb_interior_inspecciones WHERE id_inspeccion = ?");
$stmt->execute([$inspectionId]);
if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $buttonState = array_filter(explode(', ', $row['closets'])); // Eliminar valores vacíos
}
...
// Sustituciones
$titulo = "Closet condition";
$moduls = 26;

if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_FILES['photo'])) {
        $inspectionId = $_POST['inspectionId'];
        $folderName = "img/photo_gallery/{$inspectionId}/Closet_condition";
    ...
    if (isset($_POST['activeButtons'])) {
        ...
        try {
            // Sustituciones
            $stmt = $pdo->prepare("UPDATE tb_interior_inspecciones SET closets = ? WHERE id_inspeccion = ?");
    ...
    if (isset($_POST['add_note'])) {
        ...
        if (!empty($note)) {
            // Sustituciones
            $stmt = $pdo->prepare("SELECT notas_int_closets FROM tb_interior_inspecciones WHERE id_inspeccion = ?");
            $stmt->execute([$inspectionId]);
            ...
            // Sustituciones
            $stmt = $pdo->prepare("UPDATE tb_interior_inspecciones SET notas_int_closets = ? WHERE id_inspeccion = ?");
...
<body>
    ...
    <div class="bg-white p-4">
        <div class="flex items-center justify-between border-b pb-3">
            ...
            <h1 id="super_titulo" class="text-xl font-semibold text-center">Closet condition</h1>
