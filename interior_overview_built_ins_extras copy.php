...
$buttonState = [];
$stmt = $pdo->prepare("SELECT built_ins_extras FROM tb_interior_inspecciones WHERE id_inspeccion = ?");
$stmt->execute([$inspectionId]);
if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $buttonState = array_filter(explode(', ', $row['built_ins_extras']));
}

$buttonOptions = ['Oven', 'Vacuum', 'Skylights', 'Garbage Disposal', 'Air Cleaner', 'Solarium', 'HR Ventilator', 'Central Air', 'Sauna', 'Security System', 'Swimming Pool', 'Garage Opener', 'Fireplace(s)', 'Dishwasher'];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_FILES['photo'])) {
        ...
        $folderName = "img/photo_gallery/{$inspectionId}/Built_ins_extras";
    ...
    if (isset($_POST['activeButtons'])) {
        ...
        try {
            $stmt = $pdo->prepare("UPDATE tb_interior_inspecciones SET built_ins_extras = ? WHERE id_inspeccion = ?");
    ...
    if (isset($_POST['add_note'])) {
        ...
        $buttonName = $_POST['buttonName'];
        if (!empty($note)) {
            $stmt = $pdo->prepare("SELECT notas_int_built_ins_extras FROM tb_interior_inspecciones WHERE id_inspeccion = ?");
            $stmt->execute([$inspectionId]);
            ...
            $stmt = $pdo->prepare("UPDATE tb_interior_inspecciones SET notas_int_built_ins_extras = ? WHERE id_inspeccion = ?");
...
<body class="h-full w-full">
    <div id="content" class="bg-white p-4 flex flex-col items-center">
        <div class="flex items-center justify-between w-full max-w-lg border-b pb-3">
            ...
            <h1 id="super_titulo" class="text-xl font-semibold text-center">Built_ins_extras</h1>
