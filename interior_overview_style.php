...
$buttonState = [];
$stmt = $pdo->prepare("SELECT style FROM tb_interior_inspecciones WHERE id_inspeccion = ?");
$stmt->execute([$inspectionId]);
if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $buttonState = array_filter(explode(', ', $row['style']));
}

$buttonOptions = ['Bungalow', '1.5 Storey', '2 Storey', '3 Storey', 'Side Split', 'Back Split', 'Apartment'];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_FILES['photo'])) {
        ...
        $folderName = "img/photo_gallery/{$inspectionId}/Style";
    ...
    if (isset($_POST['activeButtons'])) {
        ...
        try {
            $stmt = $pdo->prepare("UPDATE tb_interior_inspecciones SET style = ? WHERE id_inspeccion = ?");
    ...
    if (isset($_POST['add_note'])) {
        $note = $_POST['note'];
        if (!empty($note)) {
            $stmt = $pdo->prepare("SELECT notas_int_style FROM tb_interior_inspecciones WHERE id_inspeccion = ?");
            $stmt->execute([$inspectionId]);
            ...
            $stmt = $pdo->prepare("UPDATE tb_interior_inspecciones SET notas_int_style = ? WHERE id_inspeccion = ?");
...
<body>
    ...
    <div class="bg-white p-4">
        <div class="flex items-center justify-between border-b pb-3">
            ...
            <h1 id="super_titulo" class="text-xl font-semibold text-center">Style</h1>
        ...
        <div id="levelInputContainer" class="hidden mt-4">
            <label for="levelInput" class="block text-sm font-medium text-gray-700">Number of Levels (Not Including the Basement)</label>
            <input type="tel" id="levelInput" class="mt-1 block w-full pl-3 pr-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" pattern="\d*">
    ...
    <script>
        ...
        function toggleButton(button) {
            ...
            if (isActive) {
                ...
                if (button.textContent.trim() === 'Side Split' || button.textContent.trim() === 'Back Split') {
        ...
        saveLevelButton.addEventListener('click', function() {
            ...
            if (level && currentButton) {
                const buttonText = `${level} ${currentButton.textContent.trim()}`;
                currentButton.textContent = buttonText;
                currentButton.classList.add('bg-green-300');
                currentButton.classList.remove('bg-background', 'hover:bg-accent');
