...
$basement_levels = 0;
$query = "SELECT basement_levels FROM vt_num_pisos WHERE id_inspeccion = ?";
$stmt = $pdo->prepare($query);
$stmt->execute([$inspectionId]);
if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_FILES['photo_add']) && $_FILES['photo_add']['error'] == UPLOAD_ERR_OK) {
        ...
        $folderName = "img/photo_gallery/{$inspectionId}/Basement_room_allocation/{$room}{$photoName}";
    ...
    if (isset($_FILES['photo']) && $_FILES['photo']['error'] == UPLOAD_ERR_OK) {
        ...
        $room = htmlspecialchars(trim($_POST['room']), ENT_QUOTES, 'UTF-8');
        $photoName = htmlspecialchars(trim($_POST['photoName']), ENT_QUOTES, 'UTF-8');
        $folderName = "img/photo_gallery/{$inspectionId}/Basement_room_allocation/{$room}_{$photoName}";
        ...
        if (move_uploaded_file($_FILES['photo']['tmp_name'], $filePath)) {
            $stmt = $pdo->prepare("INSERT INTO tbl_direccion_room_allocation (id_inspeccion, photo_address_bsmt) VALUES (?, ?)");
    ...
    if (isset($_POST['add_note'])) {
        try {
            ...
            if (!empty($note)) {
                $stmt = $pdo->prepare("SELECT notas_int_bsmt_room_allocation FROM tb_interior_inspecciones WHERE id_inspeccion = ?");
                $stmt->execute([$inspectionId]);
                ...
                $stmt = $pdo->prepare("UPDATE tb_interior_inspecciones SET notas_int_bsmt_room_allocation = ? WHERE id_inspeccion = ?");
    ...
    if (isset($_POST['delete_photo'])) {
        $photoAddress = htmlspecialchars($_POST['photo_address_bsmt'], ENT_QUOTES, 'UTF-8');
        ...
        if ($fromModal) {
            ...
            if (deleteDirectory($folderPath)) {
                $stmt = $pdo->prepare("DELETE FROM tbl_direccion_room_allocation WHERE photo_address_bsmt = ?");
...
{
    $stmt = $pdo->prepare("SELECT photo_address_bsmt FROM tbl_direccion_room_allocation WHERE id_inspeccion = ? AND piso = ?");
    $stmt->execute([$inspectionId, $selectedLevel]);
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}

$photoAddresses = getPhotoAddresses($pdo, $inspectionId, isset($_GET['level']) ? htmlspecialchars($_GET['level'], ENT_QUOTES, 'UTF-8') : 'Level 1');

if (isset($_GET['get_photos']) && isset($_GET['folder_name'])) {
    $folderName = htmlspecialchars($_GET['folder_name'], ENT_QUOTES, 'UTF-8');
    $directory = "img/photo_gallery/{$inspectionId}/Basement_room_allocation/{$folderName}";
...
<head>
    ...
    <title>Basement room allocation</title>
...
<body>
    ...
    <div class="bg-white p-4">
        <div class="flex items-center justify-between border-b pb-3">
            ...
            <h1 id="super_titulo" class="text-xl font-semibold text-center">Basement room allocation</h1>
        ...
        <div class="w-full flex items-center bg-gray-300 p-2 rounded-lg shadow-md mt-8">
            <div class="relative w-full">
                <select id="level" class="block appearance-none w-full bg-gray-200 text-black border border-white hover:border-gray-500 px-4 py-2 pr-8 rounded leading-tight focus:outline-none focus:shadow-outline custom-select" onchange="filterPhotosByLevel()">
                    <?php for ($i = 1; $i <= $basement_levels; $i++) :
                        $levelOption = "Level $i"; ?>
        ...
        <div class="mt-4 space-y-2" id="roomsContainer">
            ...
            foreach ($rooms as $room) {
                ...
                foreach ($photoAddresses as $photo) {
                    if (strpos($photo['photo_address_bsmt'], $room) !== false) {
                        $lastFolder = getLastFolderName($photo['photo_address_bsmt'], $room);
                        $fullFolder = basename(dirname($photo['photo_address_bsmt']));
                        if (!in_array($lastFolder, $displayedFolders)) {
                            ...
                            echo "<span class='text-base modified-span w-1/2' data-db-value='{$photo['photo_address_bsmt']}'>* {$lastFolder}</span>";
                            echo "<div class='icon-group w-1/2'>";
                            echo '<button class="text-green-500 hover:text-green-700 add-photo-btn" data-photo-address="' . $photo['photo_address_bsmt'] . '">';
                            ...
                            echo '</svg>';
                            echo '</button>';
                            echo '<button class="ml-1 text-red-500 hover:text-red-700 delete-photo-btn" data-photo-address="' . $photo['photo_address_bsmt'] . '">';
        ...
        document.querySelectorAll('.delete-photo-btn').forEach(button => {
            button.addEventListener('click', function() {
                ...
                if (confirm('Are you sure you want to delete this photo and its folder?')) {
                    ...
                    formData.append('photo_address_bsmt', photoAddress);
    ...
    `;
        ...
        function deletePhoto(src) {
            if (confirm('Are you sure you want to delete this photo?')) {
                const formData = new FormData();
                formData.append('delete_photo', true);
                formData.append('photo_address_bsmt', src);
