...
// Sustituciones
$buttonState = [];
$stmt = $pdo->prepare("SELECT exterior_finish FROM tb_exterior_inspecciones WHERE id_inspeccion = ?");
$stmt->execute([$inspectionId]);
if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $buttonState = array_filter(explode(', ', $row['exterior_finish'])); // Eliminar valores vacíos
}

// Sustituciones
$buttonOptions = ['Brick Veneer', 'Brick', 'Siding', 'Aluminum', 'Vinyl', 'Stone', 'Stucco', 'Log', 'Insulbrick', 'Hardboard/Fibre Cement', 'Wood Shingle'];
...
// Sustituciones
$titulo = "Exterior Finish";
$moduls = 1;

if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_FILES['photo'])) {
        $inspectionId = $_POST['inspectionId'];
        $folderName = "img/photo_gallery/{$inspectionId}/Exterior_Finish";
    ...
    if (isset($_POST['activeButtons'])) {
        ...
        try {
            // Sustituciones
            $stmt = $pdo->prepare("UPDATE tb_exterior_inspecciones SET exterior_finish = ? WHERE id_inspeccion = ?");
    ...
    if (isset($_POST['add_note'])) {
        ...
        if (!empty($note)) {
            // Sustituciones
            $stmt = $pdo->prepare("SELECT notas_ext_fin FROM tb_exterior_inspecciones WHERE id_inspeccion = ?");
            $stmt->execute([$inspectionId]);
            ...
            // Sustituciones
            $stmt = $pdo->prepare("UPDATE tb_exterior_inspecciones SET notas_ext_fin = ? WHERE id_inspeccion = ?");
...
<body>
    ...
    <div class="bg-white p-4">
        <div class="flex items-center justify-between border-b pb-3">
            ...
            <h1 id="super_titulo" class="text-xl font-semibold text-center">Exterior Finish</h1>
