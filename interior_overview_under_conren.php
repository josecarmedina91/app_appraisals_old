...
$titulo = "Under construction or renovation";
$moduls = 38;

if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['sliders_data'])) {
        $slidersData = $_POST['sliders_data'];
        $stmt = $pdo->prepare("UPDATE tb_interior_inspecciones SET under_construction_or_renovation = ? WHERE id_inspeccion = ?");
        $stmt->execute([$slidersData, $inspectionId]);
    ...
    if (isset($_FILES['photo'])) {
        ...
        $folderName = "img/photo_gallery/{$inspectionId}/Under_construction_or_renovation";
    ...
    if (isset($_POST['add_note'])) {
        $note = $_POST['note'];
        if (!empty($note)) {
            $stmt = $pdo->prepare("SELECT notas_int_under_construction_or_renovation FROM tb_interior_inspecciones WHERE id_inspeccion = ?");
            ...
            $stmt = $pdo->prepare("UPDATE tb_interior_inspecciones SET notas_int_under_construction_or_renovation = ? WHERE id_inspeccion = ?");
...
$query = "SELECT under_construction_or_renovation FROM tb_interior_inspecciones WHERE id_inspeccion = ?";
$stmt = $pdo->prepare($query);
$stmt->execute([$inspectionId]);
$slidersData = $stmt->fetchColumn();
?>
...
<head>
    ...
    <title>Under construction or renovation</title>
...
<body>
    ...
    <div class="bg-white p-4">
        <div class="flex items-center justify-between border-b pb-3">
            ...
            <h1 id="super_titulo" class="text-xl font-semibold text-center">Under construction or renovation</h1>
        ...
        <div class="overflow-x-auto w-full mt-8">
            <div class="relative w-full overflow-auto">
                <form id="slidersForm" method="POST">
                    <table class="caption-bottom text-sm w-full">
                        <thead class="[&_tr]:border-b">
                            <tr class="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                                <th class="h-12 px-4 text-left align-middle font-medium text-white [&_:has([role=checkbox])]:pr-0 bg-gray-700 w-[50%]">
                                    Component
                                </th>
                                <th class="h-12 px-4 text-left align-middle font-medium text-white [&_:has([role=checkbox])]:pr-0 bg-gray-700 w-[50%]">
                                    % Done
                    ...
                    <input type="hidden" name="sliders_data" id="sliders_data">
                </form>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        const data = [
            "Foundation & Backfill",
            "Beams, Posts, Joists & Subfloor",
            "Framing, Seathing & Roof",
            "Doors & Windows",
            "Exterior Finish (brick, vinyl, etc.)",
            "Rough Electrical",
            "Rough Plumbing",
            "Heating Ducts or Pipes",
            "Insulation",
            "Basement Floor",
            "Heating Equipment",
            "Finished Walls",
            ...
            "Finished Carpentry",
            "Painting",
            "Landscaping/Walks/Driveway"
        ];

        const createRow = (component, index, value = 100) => `
            <tr class="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                <td class="p-4 align-middle [&_:has([role=checkbox])]:pr-0 whitespace-nowrap">${component}</td>
                <td class="p-4 align-middle [&_:has([role=checkbox])]:pr-0 flex items-center gap-2 w-full">
                    <input type="range" min="0" max="100" value="${value}" class="slider w-full ${getSliderColor(value)}" id="slider-${index}" oninput="updatePercentage(${index}, this.value)">
                    <span id="percentage-${index}">${value}%</span>
                </td>
            </tr>
        `;

        function getSliderColor(value) {
            if (value < 50) return 'slider-red';
            if (value >= 50 && value <= 75) return 'slider-blue';
            return 'slider-green';
        }

        document.getElementById('table-body').innerHTML = data.map((component, index) => createRow(component, index)).join('');

        if ('<?php echo $slidersData; ?>') {
            const slidersData = JSON.parse('[<?php echo $slidersData; ?>]');
            slidersData.forEach((item, index) => {
                const [component, value] = item.split(': ').map(str => str.replace(/["%]/g, ''));
                const slider = document.getElementById(`slider-${index}`);
                const percentage = document.getElementById(`percentage-${index}`);

                slider.value = value;
                percentage.innerText = `${value}%`;
                slider.classList.add(getSliderColor(value));
            });
        }
        ...
        function updatePercentage(index, value) {
            ...
            slider.classList.remove('slider-red', 'slider-blue', 'slider-green');
        ...
        function submitSlidersForm() {
            const slidersData = data.map((component, index) => {
                const value = document.getElementById(`slider-${index}`).value;
            ...
            fetch(window.location.href, {
                    ...
                    body: `sliders_data=${encodeURIComponent(slidersData)}`
                })
                .then(response => response.text())
                .then(result => {
                    console.log(result);
