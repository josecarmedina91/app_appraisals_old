...
$buttonState = [];
$stmt = $pdo->prepare("SELECT water_heater FROM tb_interior_inspecciones WHERE id_inspeccion = ?");
$stmt->execute([$inspectionId]);
if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $buttonState = array_unique(array_filter(explode(', ', $row['water_heater'])));
}
...
foreach ($buttonState as $option) {
    ...
    } elseif (strpos($option, 'Capacity-') === 1) {
        $capacityButtons[] = str_replace(['Capacity-', '"'], '', $option);
    } else {
    }
}
...
foreach (array_merge($typeButtons, $capacityButtons) as $option) {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_FILES['photo'])) {
        ...
        $folderName = "img/photo_gallery/{$inspectionId}/Water_heater";
    ...
    if (isset($_POST['activeButtons'])) {
        ...
        if ($activeButtons === 'null' || empty(trim($activeButtons))) {
            $activeButtons = null;
        } else {
        ...
        try {
            $stmt = $pdo->prepare("UPDATE tb_interior_inspecciones SET water_heater = ? WHERE id_inspeccion = ?");
    ...
    if (isset($_POST['add_note'])) {
        $note = $_POST['note'];
        if (!empty($note)) {
            $stmt = $pdo->prepare("SELECT notas_int_water_heater FROM tb_interior_inspecciones WHERE id_inspeccion = ?");
            $stmt->execute([$inspectionId]);
            ...
            $stmt = $pdo->prepare("UPDATE tb_interior_inspecciones SET notas_int_water_heater = ? WHERE id_inspeccion = ?");
...
<body>
    ...
    <div class="bg-white p-4">
        <div class="flex items-center justify-between border-b pb-3">
            ...
            <h1 id="super_titulo" class="text-xl font-semibold text-center">Water heater</h1>
        ...
        <div class="grid grid-cols-2 gap-4 mt-4">
            ...
            <div class="col-span-1">
                ...
                <div class="flex flex-col items-center">
                    <?php
                    $numberOfPanes = ['Natural Gas', 'Electric', 'Propane', 'Tankless/On Demand'];
            ...
            <div class="col-span-1">
                <h3 class="text-lg font-semibold text-center p-1">Capacity:</h3>
                <div class="flex flex-col items-center">
                    <!-- Botones de capacidad existentes -->
                    <?php
                    $frameTypes = ['Not Applicable'];
                    foreach ($frameTypes as $option) {
                        $activeClass = in_array($option, $capacityButtons) ? 'bg-green-300' : 'bg-background';
                        echo '<button onclick="toggleButton(this)" class="' . $activeClass . ' w-full inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border border-input hover:bg-accent hover:text-accent-foreground h-10 px-4 py-2 mb-2">';
                        echo htmlspecialchars($option);
                        echo '</button>';
                    }
                    foreach ($capacityButtons as $option) {
                    ...
                    <div class="flex w-full items-center">
                        ...
                        <select id="unitSelector" class="ml-2 hidden">
                            <option value="Gallons">Select</option>
    ...
    <script>
            ...
            if (event.key === 'Enter') {
                ...
                if (otherInputValue) {
                    ...
                    unitSelector.addEventListener('change', function() {
                        addOtherOption(otherInputValue + ' ' + this.value, 'otherInput', 'Capacity-');
                        this.value = '';
        ...
        function saveChanges(customButton) {
            ...
            buttons.forEach(button => {
                if (button.classList.contains('bg-green-300')) {
                    const buttonText = button.textContent.trim();
                    if (!typeButtons.includes(buttonText) && !capacityButtons.includes(buttonText)) {
                        if (button.parentElement.previousElementSibling.textContent.trim() === 'Type:') {
                            typeButtons.push('Type-' + buttonText);
                        } else if (button.parentElement.previousElementSibling.textContent.trim() === 'Capacity:') {
                            capacityButtons.push('Capacity-' + buttonText);
                        }
                    }
                }
            });
            ...
            if (customButton) {
                const customButtonPrefix = customButton.startsWith('Type-') ? 'Type-' : 'Capacity-';
                typeButtons = typeButtons.filter(button => !button.includes(customButton.replace(customButtonPrefix, '')));
                capacityButtons = capacityButtons.filter(button => !button.includes(customButton.replace(customButtonPrefix, '')));
                if (customButtonPrefix === 'Type-') {
                    typeButtons.push(customButton);
                } else {
                    capacityButtons.push(customButton);
                }
            }

            const allButtons = [...new Set([...typeButtons, ...capacityButtons])].map(button => '"' + button + '"');
            const activeButtons = allButtons.length > 0 ? allButtons.join(', ') : null;
            const formData = new FormData();
            formData.append('activeButtons', activeButtons);
            formData.append('id', '<?php echo $inspectionId; ?>');
