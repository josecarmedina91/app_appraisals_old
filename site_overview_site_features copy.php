...
$buttonState = [];
$stmt = $pdo->prepare("SELECT site_features FROM tb_site_inspecciones WHERE id_inspeccion = ?");
$stmt->execute([$inspectionId]);
if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $buttonState = array_filter(explode(', ', $row['site_features']));
}

$buttonOptions = ['Ravine', 'River', 'Creek', 'Pond', 'Forest', 'Waterfront', 'Beach'];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_FILES['photo'])) {
        ...
        $folderName = "img/photo_gallery/{$inspectionId}/Site_features";
    ...
    if (isset($_POST['activeButtons'])) {
        ...
        try {
            $stmt = $pdo->prepare("UPDATE tb_site_inspecciones SET site_features = ? WHERE id_inspeccion = ?");
    ...
    if (isset($_POST['add_note'])) {
        ...
        $buttonName = $_POST['buttonName'];
        if (!empty($note)) {
            $stmt = $pdo->prepare("SELECT notas_site_site_features FROM tb_site_inspecciones WHERE id_inspeccion = ?");
            $stmt->execute([$inspectionId]);
            ...
            $stmt = $pdo->prepare("UPDATE tb_site_inspecciones SET notas_site_site_features = ? WHERE id_inspeccion = ?");
...
<body class="h-full w-full">
    <div id="alertMessageContainer" class="flex items-center justify-center min-h-screen">
        <div class="max-w-sm mx-auto">
            <div class="relative p-4 mt-4 bg-white rounded-lg shadow-lg">
                <div class="flex items-center justify-between space-x-2">
                    <div class="flex items-center space-x-2">
                        ...
                        <div class="text-sm font-semibold text-gray-800">MESSAGES</div>
                    </div>
                    <button class="text-gray-500 hover:text-gray-700" onclick="hideAlertMessage()">
                ...
                <div class="mt-2">
                    <div class="text-base text-gray-700">
                        Please take additional photos if all aspects of the site feature are not fully captured in the first photo.
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div id="content" class="bg-white p-4 flex flex-col items-center">
        <div class="flex items-center justify-between w-full max-w-lg border-b pb-3">
            ...
            <h1 id="super_titulo" class="text-xl font-semibold text-center">Site features</h1>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (previousPage.includes('site_overview_menu.php')) {
