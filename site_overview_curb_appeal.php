...
// Sustituciones
$buttonState = [];
$stmt = $pdo->prepare("SELECT curb_appeal FROM tb_site_inspecciones WHERE id_inspeccion = ?");
$stmt->execute([$inspectionId]);
if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $buttonState = array_filter(explode(', ', $row['curb_appeal'])); // Eliminar valores vacíos
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_FILES['photo'])) {
        ...
        $folderName = "img/photo_gallery/{$inspectionId}/Curb_appeal";
    ...
    if (isset($_POST['activeButtons'])) {
        ...
        try {
            // Sustituciones
            $stmt = $pdo->prepare("UPDATE tb_site_inspecciones SET curb_appeal = ? WHERE id_inspeccion = ?");
    ...
    if (isset($_POST['add_note'])) {
        ...
        if (!empty($note)) {
            // Sustituciones
            $stmt = $pdo->prepare("SELECT notas_site_curb_appeal FROM tb_site_inspecciones WHERE id_inspeccion = ?");
            $stmt->execute([$inspectionId]);
            ...
            // Sustituciones
            $stmt = $pdo->prepare("UPDATE tb_site_inspecciones SET notas_site_curb_appeal = ? WHERE id_inspeccion = ?");
...
<body>
    ...
    <div class="bg-white p-4">
        <div class="flex items-center justify-between border-b pb-3">
            ...
            <h1 id="super_titulo" class="text-xl font-semibold text-center">Curb appeal</h1>
