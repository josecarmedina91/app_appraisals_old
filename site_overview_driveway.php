...
$buttonState = [];
$stmt = $pdo->prepare("SELECT driveway, select_option_driveway FROM tb_site_inspecciones WHERE id_inspeccion = ?");
$stmt->execute([$inspectionId]);
if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $buttonState = array_unique(array_filter(explode(', ', $row['driveway'])));
    $selectedOption = $row['select_option_driveway'];
}

$showInitialMessage = false;
$stmt = $pdo->prepare("SELECT driveway FROM tb_site_inspecciones WHERE id_inspeccion = ?");
$stmt->execute([$inspectionId]);
if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $buttonState = array_filter(explode(', ', $row['driveway']));
    if (empty($row['driveway']) || in_array('None', $buttonState)) {
...
$buttonOptions = ['Single', 'Double', 'Triple', 'Asphalt', 'Gravel', 'Interlock'];

$buttonState = array_map(function ($option) {
    return trim($option, '"');
}, $buttonState);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_FILES['photo'])) {
        ...
        $folderName = "img/photo_gallery/{$inspectionId}/Driveway";
    ...
    if (isset($_POST['activeButtons'])) {
        ...
        try {
            $stmt = $pdo->prepare("UPDATE tb_site_inspecciones SET driveway = ? WHERE id_inspeccion = ?");
    ...
    if (isset($_POST['selectOptionDriveway'])) {
        $selectedOption = $_POST['selectOptionDriveway'];
        $inspectionId = $_POST['id'];

        try {
            $stmt = $pdo->prepare("UPDATE tb_site_inspecciones SET select_option_driveway = ? WHERE id_inspeccion = ?");
            $stmt->execute([$selectedOption, $inspectionId]);
            exit;
        } catch (PDOException $e) {
            die("ERROR: No se pudo actualizar. " . $e->getMessage());
        }
    ...
    if (isset($_POST['add_note'])) {
        $note = $_POST['note'];
        if (!empty($note)) {
            $stmt = $pdo->prepare("SELECT notas_site_driveway FROM tb_site_inspecciones WHERE id_inspeccion = ?");
            $stmt->execute([$inspectionId]);
            ...
            $stmt = $pdo->prepare("UPDATE tb_site_inspecciones SET notas_site_driveway = ? WHERE id_inspeccion = ?");
...
<body>
    <div id="initialMessage" class="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center <?php echo $showInitialMessage ? '' : 'hidden'; ?>">
        <div class="bg-white p-6 rounded-lg shadow-lg text-center max-w-sm mx-auto">
            <p class="text-base font-semibold mb-4">What type of driveway does the property have?</p>
            <div class="flex justify-center gap-4">
                <button onclick="handleResponse('None')" class="flex items-center justify-center bg-red-500 text-white px-4 py-2 rounded-md hover:bg-red-600">
                    None
                </button>
                <button onclick="handleResponse('Private')" class="flex items-center justify-center bg-green-500 text-white px-4 py-2 rounded-md hover:bg-green-600">
                    Private
                </button>
                <button onclick="handleResponse('Mutual')" class="flex items-center justify-center bg-green-500 text-white px-4 py-2 rounded-md hover:bg-green-600">
                    Mutual
                </button>
            </div>
        </div>
    ...
    <div id="content" class="<?php echo $showInitialMessage ? 'hidden' : ''; ?>">
        ...
        <div class="bg-white p-4">
            <div class="flex items-center justify-between border-b pb-3">
                ...
                <h1 id="super_titulo" class="text-xl font-semibold text-center">Driveway</h1>
            ...
            <div class="mt-4 border-b pb-3">
                <h2 class="text-lg font-semibold">Selected option:
                    <select id="drivewaySelect" onchange="updateDrivewayOption()">
                        <option value="None" <?php echo ($selectedOption == 'None') ? 'selected' : ''; ?>>None</option>
                        <option value="Private" <?php echo ($selectedOption == 'Private') ? 'selected' : ''; ?>>Private</option>
                        <option value="Mutual" <?php echo ($selectedOption == 'Mutual') ? 'selected' : ''; ?>>Mutual</option>
                    </select>
                </h2>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4 mt-4">
                ...
                <div class="col-span-1">
                    <h3 class="text-lg font-semibold text-center p-1">Cars:</h3>
                    <div class="flex flex-col items-center">
                        <?php
                        $cars = ['Single', 'Double', 'Triple', 'Quadruple', 'Quintuple'];
                        $renderedButtons = [];
                ...
                <div class="col-span-1">
                    <h3 class="text-lg font-semibold text-center p-1">Surface:</h3>
                    <div class="flex flex-col items-center">
                        <?php
                        $surface = ['Asphalt', 'Gravel', 'Interlock'];
                        foreach ($surface as $option) {
                            $activeClass = in_array($option, $buttonState) ? 'bg-green-300' : 'bg-background';
                            echo '<button onclick="toggleButton(this)" class="' . $activeClass . ' w-full inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border border-input hover:bg-accent hover:text-accent-foreground h-10 px-4 py-2 mb-2">';
                            echo htmlspecialchars($option);
                            echo '</button>';
                        ...
                        foreach ($buttonState as $option) {
                            if (!in_array($option, $renderedButtons)) {
    ...
    <script>
        ...
        function handleResponse(response) {
            ...
            formData.append('selectOptionDriveway', response);
            ...
            fetch(window.location.href, {
                ...
                .then(data => {
                    if (response === 'None') {
                        ...
                        document.querySelector('#drivewaySelect').value = response;
                        document.querySelector('.text-lg.font-semibold').innerHTML = 'Selected option: <select id="drivewaySelect" onchange="updateDrivewayOption()">' +
                            '<option value="None"' + (response === 'None' ? ' selected' : '') + '>None</option>' +
                            '<option value="Private"' + (response === 'Private' ? ' selected' : '') + '>Private</option>' +
                            '<option value="Mutual"' + (response === 'Mutual' ? ' selected' : '') + '>Mutual</option>' +
                            '</select>';
                    }
                })
        ...
        function updateDrivewayOption() {
    const selectedOption = document.getElementById('drivewaySelect').value;
    const formData = new FormData();
    formData.append('selectOptionDriveway', selectedOption);
    formData.append('id', '<?php echo $inspectionId; ?>');
    ...
    .then(data => {
        if (selectedOption === 'None') {
            const buttons = document.querySelectorAll('button.bg-green-300');
            ...
            updateFormData.append('activeButtons', '');
            updateFormData.append('id', '<?php echo $inspectionId; ?>');
            ...
            console.log('Driveway option updated');
        }
    })
    .catch(error => console.error('Error:', error));
}
