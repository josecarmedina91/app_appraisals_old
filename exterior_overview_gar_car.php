...
$buttonState = [];
$stmt = $pdo->prepare("SELECT garage FROM tb_exterior_inspecciones WHERE id_inspeccion = ?");
$stmt->execute([$inspectionId]);
if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $buttonState = array_unique(array_filter(explode(', ', $row['garage'])));
}
...
foreach ($buttonState as $option) {
    ...
    } elseif (strpos($option, 'Source-') === 1) {
        $sourceButtons[] = str_replace(['Source-', '"'], '', $option);
    } else {
    }
}
...
foreach (array_merge($typeButtons, $sourceButtons) as $option) {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_FILES['photo'])) {
        ...
        $folderName = "img/photo_gallery/{$inspectionId}/Garage";
    ...
    if (isset($_POST['activeButtons'])) {
        ...
        try {
            $stmt = $pdo->prepare("UPDATE tb_exterior_inspecciones SET garage = ? WHERE id_inspeccion = ?");
    ...
    if (isset($_POST['add_note'])) {
        $note = $_POST['note'];
        if (!empty($note)) {
            $stmt = $pdo->prepare("SELECT notas_ext_garage FROM tb_exterior_inspecciones WHERE id_inspeccion = ?");
            $stmt->execute([$inspectionId]);
            ...
            $stmt = $pdo->prepare("UPDATE tb_exterior_inspecciones SET notas_ext_garage = ? WHERE id_inspeccion = ?");
...
<body>
    ...
    <div class="bg-white p-4">
        <div class="flex items-center justify-between border-b pb-3">
            ...
            <h1 id="super_titulo" class="text-xl font-semibold text-center">Garage</h1>
        ...
        <div class="grid grid-cols-2 gap-4 mt-4">
            ...
            <div class="col-span-1">
                ...
                <div class="flex flex-col items-center">
                    <?php
                    $numberOfPanes = ['Single', 'Double', 'Triple', 'Four', 'Five'];
            ...
            <div class="col-span-1">
                <h3 class="text-lg font-semibold text-center p-1">Garage:</h3>
                <div class="flex flex-col items-center">
                    <?php
                    $frameTypes = ['Detached', 'Attached', 'Built-In', 'Underground'];
                    foreach ($frameTypes as $option) {
                        $activeClass = in_array($option, $sourceButtons) ? 'bg-green-300' : 'bg-background';
                        echo '<button onclick="toggleButton(this)" class="' . $activeClass . ' w-full inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border border-input hover:bg-accent hover:text-accent-foreground h-10 px-4 py-2 mb-2">';
                        echo htmlspecialchars($option);
                        echo '</button>';
                    }
                    foreach ($sourceButtons as $option) {
    ...
    <script>
        ...
        function saveAndGoBack() {
            ...
            buttons.forEach(button => {
                if (button.classList.contains('bg-green-300')) {
                    const prefix = button.closest('.col-span-1').querySelector('h3').textContent.includes('Type') ? 'Type-' : 'Source-';
                    activeButtons.push('"' + prefix + button.textContent.trim() + '"');
                }
            });
            ...
            fetch(window.location.href, {
                ...
                .then(data => {
                    window.location.href = 'exterior_overview_menu.php?id=<?php echo $inspectionId; ?>';
                })
        ...
        document.getElementById('otherInput').addEventListener('keypress', function(event) {
            if (event.key === 'Enter') {
                ...
                if (otherInputValue) {
                    addOtherOption(otherInputValue, 'otherInput', 'Source-');
                    this.value = '';
                }
            }
        });
        ...
        function saveChanges(customButton) {
            ...
            buttons.forEach(button => {
                if (button.classList.contains('bg-green-300')) {
                    const buttonText = button.textContent.trim();
                    if (!typeButtons.includes(buttonText) && !sourceButtons.includes(buttonText)) {
                        if (button.parentElement.previousElementSibling.textContent.trim() === 'Cars:') {
                            typeButtons.push('Type-' + buttonText);
                        } else if (button.parentElement.previousElementSibling.textContent.trim() === 'Garage:') {
                            sourceButtons.push('Source-' + buttonText); // Aseguramos que aquí se procese correctamente
                        }
                    }
                }
            });
            ...
            if (customButton) {
                const customButtonPrefix = customButton.startsWith('Type-') ? 'Type-' : 'Source-';
                typeButtons = typeButtons.filter(button => !button.includes(customButton.replace(customButtonPrefix, '')));
                sourceButtons = sourceButtons.filter(button => !button.includes(customButton.replace(customButtonPrefix, '')));
                if (customButtonPrefix === 'Type-') {
                    typeButtons.push(customButton);
                } else {
                    sourceButtons.push(customButton);
                }
            }

            const allButtons = [...new Set([...typeButtons, ...sourceButtons])].map(button => '"' + button + '"');
            const formData = new FormData();
