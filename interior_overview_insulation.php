...
$buttonState = [];
$stmt = $pdo->prepare("SELECT insulation FROM tb_interior_inspecciones WHERE id_inspeccion = ?");
$stmt->execute([$inspectionId]);
if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $buttonState = array_unique(array_filter(explode(', ', $row['insulation'])));
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_FILES['photo'])) {
        ...
        $folderName = "img/photo_gallery/{$inspectionId}/Insulation";
    ...
    if (isset($_POST['activeButtons'])) {
        ...
        try {
            $stmt = $pdo->prepare("UPDATE tb_interior_inspecciones SET insulation = ? WHERE id_inspeccion = ?");
    ...
    if (isset($_POST['add_note'])) {
        $note = $_POST['note'];
        if (!empty($note)) {
            $stmt = $pdo->prepare("SELECT notas_int_insulation FROM tb_interior_inspecciones WHERE id_inspeccion = ?");
            $stmt->execute([$inspectionId]);
            ...
            $stmt = $pdo->prepare("UPDATE tb_interior_inspecciones SET notas_int_insulation = ? WHERE id_inspeccion = ?");
...
<body>
    ...
    <div class="bg-white p-4">
        <div class="flex items-center justify-between border-b pb-3">
            ...
            <h1 id="super_titulo" class="text-xl font-semibold text-center">Insulation</h1>
        ...
        <div class="grid grid-cols-2 gap-4 mt-4">
            ...
            <div class="col-span-1">
                ...
                <div class="flex flex-col items-center">
                    <?php
                    $numberOfPanes = ['Ceiling', 'Walls', 'Basement', 'Crawl Space'];
            ...
            <div class="col-span-1">
                <h3 class="text-lg font-semibold text-center p-1">Source:</h3>
                <div class="flex flex-col items-center">
                    <?php
                    $frameTypes = ['Observed', 'Owner/Occupant', 'Assumed'];
            ...
            buttons.forEach(button => {
                if (button.classList.contains('bg-green-300')) {
                    ...
                    if (!typeButtons.includes(buttonText) && !sourceButtons.includes(buttonText)) {
                        ...
                        } else if (button.parentElement.previousElementSibling.textContent.trim() === 'Source:') {
