...
// Sustituciones
$buttonState = [];
$stmt = $pdo->prepare("SELECT type_of_building FROM tb_exterior_inspecciones WHERE id_inspeccion = ?");
$stmt->execute([$inspectionId]);
if ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $buttonState = array_filter(explode(', ', $row['type_of_building'])); // Eliminar valores vacíos
}

// Sustituciones
$buttonOptions = ['Detached', 'Apartment', 'Semi-Det', 'Townhouse'];
...
// Sustituciones
$titulo = "Type of Building";
$moduls = 3;

if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_FILES['photo'])) {
        $inspectionId = $_POST['inspectionId'];
        $folderName = "img/photo_gallery/{$inspectionId}/Type_of_Building";
    ...
    if (isset($_POST['activeButtons'])) {
        ...
        try {
            // Sustituciones
            $stmt = $pdo->prepare("UPDATE tb_exterior_inspecciones SET type_of_building = ? WHERE id_inspeccion = ?");
    ...
    if (isset($_POST['add_note'])) {
        ...
        if (!empty($note)) {
            // Sustituciones
            $stmt = $pdo->prepare("SELECT notas_ext_type_of_bu FROM tb_exterior_inspecciones WHERE id_inspeccion = ?");
            $stmt->execute([$inspectionId]);
            ...
            // Sustituciones
            $stmt = $pdo->prepare("UPDATE tb_exterior_inspecciones SET notas_ext_type_of_bu = ? WHERE id_inspeccion = ?");
