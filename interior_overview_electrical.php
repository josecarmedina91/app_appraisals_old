...
class Inspection {
    ...
    public function getElectricalOptions($inspectionId) {
        $stmt = $this->pdo->prepare("SELECT electrical FROM tb_interior_inspecciones WHERE id_inspeccion = ?");
        $stmt->execute([$inspectionId]);
        return array_filter(explode(', ', $stmt->fetch(PDO::FETCH_ASSOC)['electrical']));
    }

    public function updateElectricalOptions($activeButtons, $inspectionId) {
        $stmt = $this->pdo->prepare("UPDATE tb_interior_inspecciones SET electrical = ? WHERE id_inspeccion = ?");
        $stmt->execute([$activeButtons, $inspectionId]);
    }

    public function addNoteToInspection($note, $inspectionId) {
        $stmt = $this->pdo->prepare("SELECT notas_int_electrical FROM tb_interior_inspecciones WHERE id_inspeccion = ?");
        $stmt->execute([$inspectionId]);
        ...
        $stmt = $this->pdo->prepare("UPDATE tb_interior_inspecciones SET notas_int_electrical = ? WHERE id_inspeccion = ?");
...
$inspection = new Inspection($database->pdo);

$usuario_id = $_SESSION['usuario_id'];
$userDetails = $user->getUserDetails($usuario_id);

if (!$userDetails) {
    die('Usuario no encontrado.');
}

$nombre_usuario = $userDetails['nombre_completo'];
$correo_usuario = $userDetails['correo_electronico'];
...
$buttonState = $inspection->getElectricalOptions($inspectionId);
$buttonOptions = ['Fuses', 'Breakers', 'Generator', 'Solar panel'];

foreach ($buttonState as $option) {
    $option = trim($option, '"');
    if (!in_array($option, $buttonOptions) && $option !== 'No electrical service') {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_FILES['photo'])) {
        ...
        $folderName = "img/photo_gallery/{$inspectionId}/Electrical";
    ...
    if (isset($_POST['activeButtons'])) {
        ...
        $inspection->updateElectricalOptions($activeButtons, $inspectionId);
    ...
    if (isset($_POST['add_note'])) {
        $note = $_POST['note'];
        if (!empty($note)) {
            $inspection->addNoteToInspection($note, $inspectionId);
...
<body>
    ...
    <div class="bg-white p-4">
        <div class="flex items-center justify-between border-b pb-3">
            ...
            <h1 id="super_titulo" class="text-xl font-semibold text-center">Electrical</h1>
        ...
        <div class="grid grid-cols-2 gap-4 mt-4">
            ...
            foreach ($buttonOptions as $option) {
                $activeClass = in_array('"' . $option . '"', $buttonState) ? 'bg-green-300' : 'bg-background hover:bg-accent';
                echo '<button onclick="toggleButton(this)" data-option="' . htmlspecialchars($option) . '" class="' . $activeClass . ' inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border border-input h-10 px-4 py-2">';
                echo htmlspecialchars($option);
                echo '</button>';
            }
            ?>

            <button id="noElectricalService" data-option="No electrical service" class="<?php echo in_array('"No electrical service"', $buttonState) ? 'bg-green-300' : 'bg-background hover:bg-accent'; ?> inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border border-input h-10 px-4 py-2">
                No electrical service
            </button>

            <input type="text" id="otherInput" placeholder="Others" class="hidden inline-flex text-center items-center justify-center whitespace-nowrap rounded-md text-base font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 border border-input h-10 px-4 py-2">
        </div>

        <div id="levelInputContainer" class="hidden mt-4">
            <label for="levelInput" class="block text-sm font-medium text-gray-700">Capacity (amps)</label>
            <input type="tel" id="levelInput" class="mt-1 block w-full pl-3 pr-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" inputmode="numeric" pattern="\d*" oninput="validateNumberInput(this)">
            <div class="flex mt-2 space-x-2">
                <button id="saveLevelButton" class="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-blue-500 hover:bg-blue-600 text-white h-10 px-4 py-2">Save</button>
                <button id="cancelLevelButton" class="inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-red-500 hover:bg-red-600 text-white h-10 px-4 py-2">Cancel</button>
            </div>
        </div>
    ...
    <script>
        ...
        const cancelLevelButton = document.getElementById('cancelLevelButton');
        const noElectricalServiceButton = document.getElementById('noElectricalService');
        ...
        function toggleButton(button) {
            ...
            if (isActive) {
                ...
                if (option === 'Generator' || option === 'Fuses' || option === 'Breakers' || option === 'Solar panel') {
                    currentButton = button;
                    levelInput.value = '';
                    levelInputContainer.classList.remove('hidden');
                    levelInput.focus();
                    return;
                } else {
        ...
        noElectricalServiceButton.addEventListener('click', function() {
            this.classList.toggle('bg-green-300');
            this.classList.toggle('bg-background');
            saveChanges();
        });

        saveLevelButton.addEventListener('click', function() {
            const level = levelInput.value.trim();
            if (level && currentButton) {
                const option = currentButton.getAttribute('data-option');
                const buttonText = `${option} cap ${level} A`;
                ...
                newButton.setAttribute('data-option', option);
                newButton.setAttribute('onclick', 'toggleButton(this)');
                document.querySelector('.grid-cols-2').insertBefore(newButton, document.getElementById('otherInput'));

                levelInputContainer.classList.add('hidden');
                currentButton = null;
        ...
        cancelLevelButton.addEventListener('click', function() {
            levelInputContainer.classList.add('hidden');
            currentButton = null;
            document.documentElement.scrollTop = previousScrollTop;
        });
        ...
        window.addEventListener('click', function(event) {
            if (levelInputContainer.classList.contains('hidden')) return;
            if (!levelInputContainer.contains(event.target) && event.target !== currentButton && event.target !== saveLevelButton) {
                levelInputContainer.classList.add('hidden');
                currentButton = null;
                document.documentElement.scrollTop = previousScrollTop;
            }
        ...
        function saveAndGoBack() {
            ...
            const buttons = document.querySelectorAll('button[onclick="toggleButton(this)"], #noElectricalService');
            let activeButtons = [];
